...
namespace SqliteHelper
{
    public class SqliteDatabaseExporter
    {
        ...
        public static bool CreateDatabaseWithSchemaAndData(DatabaseProject project, string databasePath, out string errorMessage)
        {
            errorMessage = string.Empty;
            ...
            try
            {
                ...
                if (!CreateDatabaseWithSchema(project, databasePath, out errorMessage))
                {
                    return false;
                }

                // Since this is a design tool and doesn't store actual data,
                // we'll insert sample data for each table
                var connectionString = $"Data Source={databasePath};";
                ...
                using (var connection = new SqliteConnection(connectionString))
                {
                    ...
                    foreach (var table in project.Tables)
                    {
                        ...
                        var insertStatement = GenerateSampleInsert(table);
            ...
            {
                errorMessage = $"Error creating database with data: {ex.Message}";
